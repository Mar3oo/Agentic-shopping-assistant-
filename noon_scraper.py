...
class NoonScraper:
    ...
    def _extract_product_details(self, page, url):
        ...
        product["details_text"] = self._extract_details_from_json_ld(page, product["title"])
                        ...
                        if product_page_response and product_page_response.status == 200:
                            try:
                                product_page.wait_for_selector(
                                    "main h1, span[class*='ProductTitle'], h1", timeout=3000
                                )
                            except PlaywrightTimeoutError:
                                pass
                            extracted_product = self._extract_product_details(product_page, url)
